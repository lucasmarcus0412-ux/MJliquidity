...
export default function ProMarketsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      loadPosts();
      loadMessages();
      getHiddenMessages('four_markets').then(setHiddenMessageIds).catch(() => {});
  ...
  const handleReportMessage = async (messageId: string) => {
    ...
    if (Platform.OS === 'web') {
      ...
      try {
        await reportMessage(messageId, displayName, 'four_markets', reason || undefined);
      ...
      Alert.alert('Report Message', 'Report this message to admins?', [
        ...
        { text: 'Report', style: 'destructive', onPress: async () => {
          try {
            await reportMessage(messageId, displayName, 'four_markets');
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
            Alert.alert('Reported', 'This message has been reported to the admins.');
          } catch { Alert.alert('Error', 'Failed to report message.'); }
        }},
  ...
  const handleHideMessage = async (messageId: string) => {
    try {
      await hideMessage(messageId, 'four_markets');
