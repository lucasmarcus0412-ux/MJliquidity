...
const CATEGORY_ICONS: Record<Category, { name: string; family: 'ionicons' | 'feather' }> = {
  general: { name: 'globe-outline', family: 'ionicons' },
  forex: { name: 'swap-horizontal', family: 'ionicons' },
  indices: { name: 'trending-up', family: 'feather' },
  crypto: { name: 'logo-bitcoin', family: 'ionicons' },
  commodities: { name: 'diamond-outline', family: 'ionicons' },
};

function getCategoryIcon(cat: Category, size: number, color: string) {
    ...
    <View style={[styles.card, { backgroundColor: c.card, borderColor: c.cardBorder }]}>
      <View style={styles.cardHeader}>
        <View style={[styles.categoryBadge, { backgroundColor: c.goldMuted }]}>
          {getCategoryIcon(post.category, 14, c.gold)}
      ...
      <View style={styles.cardFooter}>
        <View style={styles.adminBadge}>
          ...
          <Text style={[styles.adminLabel, { color: c.goldLight }]}>MJ Liquidity</Text>
        </View>
      </View>
    </View>
  );
...
export default function AnalysisFeed() {
  ...
  const loadPosts = useCallback(async () => {
    const data = await getAnalysisPosts();
    setPosts(data);
  }, []);
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12 + webTopInset }]}>
        <View>
          <Text style={[styles.headerTitle, { color: c.gold }]}>Analysis</Text>
          <Text style={[styles.headerSubtitle, { color: c.textMuted }]}>Intraday Updates</Text>
        </View>
        {isAdmin && (
          <Pressable
            onPress={() => {
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyText, { color: c.textMuted }]}>
              Market analysis and updates will appear here
            </Text>
          </View>
        }
      />
          ...
          <View style={[styles.modalOverlay]}>
            <View style={[styles.composeModal, { backgroundColor: c.surface }]}>
              <View style={styles.composeHeader}>
                ...
                <Text style={[styles.composeTitle, { color: c.text }]}>New Analysis</Text>
                <Pressable
                  onPress={handlePost}
                  style={[styles.postBtn, { backgroundColor: c.gold }]}
                >
              ...
              <View style={styles.categoryRow}>
                {CATEGORIES.map((cat) => (
                  ...
                  >
                    {getCategoryIcon(cat, 12, selectedCategory === cat ? c.gold : c.textMuted)}
                    <Text
                      style={[
                        styles.chipText,
                        { color: selectedCategory === cat ? c.gold : c.textMuted },
                      ]}
                    >
